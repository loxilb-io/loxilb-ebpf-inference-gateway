...
/* --- Engine-dialect ops wrappers (SGLang) --------------------------------
 * The SGLang dual-dispatch machine still lives in this file; these wrappers
 * adapt it to the pd_dialect_ops table (sockproxy_pd.h) so the forward path
 * calls through the epval's cached ops pointer instead of testing the
 * engine. They migrate to sockproxy_pd_sglang.c with the machine (the vLLM
 * counterparts already live in sockproxy_pd_vllm.c). Bodies are verbatim
 * moves of the former inline branches. */
