...
typedef struct notify_cbs {
  ...
  /* Phase 93-05 (R1, bounded admission resume): invoked ON THE OWNER WORKER when a
...
/* Phase 90 (conc=128 single-owner teardown): remove a fd's poll/earr entry
...
/* Phase 89 Option A: like notify_add_ent but pins fd to pin_fd's notify worker. */
int notify_add_ent_pinned(void *ctx, int fd, notify_type_t type, void *priv, uint64_t gen, int pin_fd);
/* Phase 93-05 (R1): which notify worker owns `fd` (its `fd % n_thrs` shard, or the
...
/* Phase 93-05 (R1): the registered priv (pfe) for `fd` + its registration gen, read
...
/* Phase 93-05 (R1): enqueue `fd` onto worker `thr_id`'s MPSC resume-pending queue and
