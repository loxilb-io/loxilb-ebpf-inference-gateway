...
proxy_metrics_snapshot_t proxy_get_metrics(void) {
    ...
    snapshot.hdr_deadline_drops         = atomic_load(&global_stats.hdr_deadline_drops);
    ...
    /* TRT-LLM sequential-dialect counters */
    snapshot.pd_trt_ctx_early_exit      = atomic_load(&global_stats.pd_trt_ctx_early_exit);
