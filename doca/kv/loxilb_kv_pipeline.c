...
/*
 * loxilb_kv_pipeline.c -- Session orchestrator for the KV cache pipeline.
 *
 * Central component that drives each KV transfer session through the
 * 4-stage state machine: TCP recv -> decompress -> DMA to GPU -> done.
 * Handles priority eviction with >50% completion protection and draining
 * sub-state for in-flight PE tasks.
 *
 * Write-back path: DMA from GPU -> compress -> TCP send.
 *
 * Build: HAVE_DOCA=1 (uses DOCA mmap for GPU mmap lifecycle).
 */
...
/* Max decompressed chunk size (4 MB) */
#define LLB_KV_MAX_CHUNK_DECOMPRESSED   (4 * 1024 * 1024)

/* Max compressed chunk payload buffer (4 MB) */
#define LLB_KV_MAX_CHUNK_COMPRESSED     (4 * 1024 * 1024)

/* Pipeline main loop sleep (microseconds) */
#define LLB_KV_PIPELINE_POLL_US         100

/* Protection threshold: sessions >50% done cannot be evicted */
#define LLB_KV_EVICTION_PROTECT_PCT     50

/* ------------------------------------------------------------------ */
/* Internal session struct (extends kv_session_t from header)          */
/* ------------------------------------------------------------------ */

typedef struct {
    kv_session_t        base;             /* session_id, state, priority, etc.  */
    llb_kv_comch_msg_t  req;              /* original request message           */
    void               *tcp_conn;         /* transport connection handle        */
    llb_kv_chunk_hdr_t  cur_chunk;        /* current chunk being processed      */
    void               *chunk_payload;    /* current chunk compressed payload   */
    void               *decompress_buf;   /* decompressed output buffer         */
    size_t              decompress_len;   /* last decompressed output length    */
    size_t              gpu_write_offset;  /* accumulated GPU offset for DMA    */
    void               *gpu_mmap;         /* per-session GPU mmap (opaque ptr)  */
    int                 evicting;         /* 1 = being evicted, draining        */
    int                 gpu_mmap_owned;   /* 1 = we imported it, must destroy   */
} kv_session_internal_t;

/* ------------------------------------------------------------------ */
/* Pipeline context (full definition -- opaque to callers)             */
/* ------------------------------------------------------------------ */
...
struct llb_kv_pipeline_ctx {
    ...
    llb_kv_comch_ctx_t    *comch;   /* NULL if REST-only mode */
    ...
    /* Statistics (exposed to Prometheus via CGO) */
...
/* ------------------------------------------------------------------ */
/* Helper: find session by session_id                                  */
/* ------------------------------------------------------------------ */

static int
find_session_by_id(llb_kv_pipeline_ctx_t *ctx, uint64_t session_id)
{
    for (int i = 0; i < LLB_KV_MAX_SESSIONS; i++) {
        if (ctx->sessions[i].base.session_id == session_id &&
...
/* ------------------------------------------------------------------ */
/* Priority eviction: least-progress-first with >50% protection        */
/* ------------------------------------------------------------------ */
...
find_eviction_candidate(llb_kv_pipeline_ctx_t *ctx, uint32_t new_priority)
{
    ...
    for (int i = 0; i < LLB_KV_MAX_SESSIONS; i++) {
        ...
        /* Skip idle, already-evicting, and error sessions */
        if (s->base.state == KV_SESSION_IDLE ||
            s->base.state == KV_SESSION_ERROR ||
            s->evicting)
            continue;

        /* Must have strictly lower priority than new request */
        if (s->base.priority >= new_priority)
            continue;

        /* Protected: sessions >50% complete cannot be evicted */
        ...
        /* Least-progress-first: pick session with lowest completion ratio */
...
/*
 * Drain in-flight PE tasks for an evicted session.
 * Poll compress and DMA PEs to let any pending tasks complete
 * before reusing the slot (RESEARCH pitfall #7).
 */
static void
drain_session(llb_kv_pipeline_ctx_t *ctx, kv_session_internal_t *sess)
{
    /*
     * We cannot track per-session PE tasks precisely in this
     * architecture. Instead, we do a brief PE progress drain
     * to allow any pending tasks to complete their callbacks.
     */
    for (int i = 0; i < 100; i++) {
        if (ctx->comch)
            llb_kv_comch_poll(ctx->comch);
        usleep(100);
    }
...
session_reset(llb_kv_pipeline_ctx_t *ctx, kv_session_internal_t *sess)
{
    ...
    /* Destroy per-session GPU mmap if we own it */
    if (sess->gpu_mmap && sess->gpu_mmap_owned) {
#ifdef HAVE_DOCA
        doca_mmap_destroy((struct doca_mmap *)sess->gpu_mmap);
#else
        free(sess->gpu_mmap);
#endif
        sess->gpu_mmap = NULL;
...
/* ------------------------------------------------------------------ */
/* Allocate session slot with eviction support                         */
/* ------------------------------------------------------------------ */
...
allocate_session(llb_kv_pipeline_ctx_t *ctx, uint32_t priority)
{
    ...
    /* No idle slot -- try priority eviction */
    idx = find_eviction_candidate(ctx, priority);
    if (idx < 0)
        return -1;  /* No evictable candidate */

    kv_session_internal_t *victim = &ctx->sessions[idx];

    /* Mark as evicting and drain */
    victim->evicting = 1;
    victim->base.state = KV_SESSION_ERROR;

    drain_session(ctx, victim);

    /* Notify vLLM that session was evicted */
    if (ctx->comch) {
        llb_kv_comch_send_done(ctx->comch, victim->base.session_id,
                               LLB_KV_COMCH_MSG_FETCH_DONE,
                               LLB_KV_ERR_EVICTED);
    }
...
/* ------------------------------------------------------------------ */
/* GPU mmap pre-registration (Option B -- called from REST handler)    */
/* ------------------------------------------------------------------ */
                                  ...
                                  size_t desc_len)
{
    ...
    idx = find_session_by_id(ctx, session_id);
    if (idx < 0) {
        /* Allocate a new slot in IDLE state to hold the pre-registered mmap */
        idx = find_idle_slot(ctx);
        if (idx < 0)
            return LLB_KV_ERR_NO_SESSION;

        ctx->sessions[idx].base.session_id = session_id;
        /* Keep state as IDLE -- will be activated by fetch_start */
    }

    kv_session_internal_t *sess = &ctx->sessions[idx];

    /* Import the GPU mmap via DMA subsystem */
...
/* ------------------------------------------------------------------ */
/* Fetch start -- triggered by KV_FETCH_REQ                            */
/* ------------------------------------------------------------------ */
                            ...
                            const llb_kv_comch_msg_t *msg)
{
    ...
    /* Check if session was pre-registered (GPU mmap already imported) */
    idx = find_session_by_id(ctx, msg->session_id);
    if (idx < 0) {
        /* Check IDLE slots that may have pre-registered mmap */
        for (int i = 0; i < LLB_KV_MAX_SESSIONS; i++) {
            if (ctx->sessions[i].base.state == KV_SESSION_IDLE &&
    ...
    if (idx < 0) {
        /* No pre-registered session -- allocate new slot */
...
/* ------------------------------------------------------------------ */
/* Write-back start -- triggered by KV_WRITEBACK_REQ                   */
/* ------------------------------------------------------------------ */
                                ...
                                const llb_kv_comch_msg_t *msg)
{
    ...
    /* Allocate session slot (separate from fetch -- non-blocking) */
    idx = allocate_session(ctx, msg->priority);
    if (idx < 0)
        return LLB_KV_ERR_NO_SESSION;
    ...
    /* Initialize session for write-back */
    memcpy(&sess->req, msg, sizeof(*msg));
    sess->base.session_id   = msg->session_id;
    sess->base.priority     = msg->priority;
    sess->base.total_chunks = msg->total_chunks;
    ...
    /* Allocate buffers for write-back path */
    if (!sess->decompress_buf) {
        sess->decompress_buf = malloc(LLB_KV_MAX_CHUNK_DECOMPRESSED);
        if (!sess->decompress_buf) {
            session_reset(ctx, sess);
...
/* ------------------------------------------------------------------ */
/* Process one session: advance through state machine                   */
/* ------------------------------------------------------------------ */
...
process_session(llb_kv_pipeline_ctx_t *ctx, kv_session_internal_t *sess)
{
    ...
    /* -- Fetch path: TCP recv -> decompress -> DMA to GPU -- */
    ...
    case KV_SESSION_DEQUANTIZE:
        {
            ...
            if (llb_kv_ring_pop(&ctx->deq_to_dma_ring, &out) != 0)
                break;  /* Workers still processing */
    ...
    case KV_SESSION_WRITEBACK_TCP:
        /* TCP connect if not already connected */
        ...
        /* Send chunk header + compressed payload */
    ...
    case KV_SESSION_WRITEBACK_DONE:
        /* Send KV_WRITEBACK_DONE via ComCh */
        if (ctx->comch) {
            llb_kv_comch_send_done(ctx->comch, sess->base.session_id,
                                   LLB_KV_COMCH_MSG_WRITEBACK_DONE,
                                   LLB_KV_OK);
        }
        session_reset(ctx, sess);
        ctx->session_count--;
...
/* ------------------------------------------------------------------ */
/* Process: advance all active sessions                                */
/* ------------------------------------------------------------------ */
...
llb_kv_pipeline_run(llb_kv_pipeline_ctx_t *ctx)
{
    ...
    while (ctx->running) {
        /* Poll ComCh for new messages */
        if (ctx->comch)
            llb_kv_comch_poll(ctx->comch);

        /* Advance all sessions */
